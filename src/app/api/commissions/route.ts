...
import { sendBespokeInquiryEmail } from "@/lib/email";
...
export async function GET() {
  ...
  try {
    ...
    console.error("[TEAK HAUS API ERROR] GET /api/commissions failed:", err);
...
const commissionSchema = z.object({
  ...
  pincode: z
    ...
    .optional(),
  project_type: z.string().optional().default("Custom Dining"),
  ...
  { message: "Full name is required (min 2 characters)", path: ["name"] }
).refine(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = err instanceof Error ? err.message : "Internal server error";
    console.error("[TEAK HAUS API ERROR] POST /api/commissions failed:", err);
